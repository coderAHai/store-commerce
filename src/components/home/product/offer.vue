<template>
  <div class="mx-auto p-10 max-w-[460px] flex flex-col border-2 border-secondary-dark rounded-lg lg:mx-4">
    <div class="mx-4 sm:mx-0">
      <!-- offer -->
      <div class="flex flex-col justify-center items-center text-center mb-6">
        <p class="uppercase mb-2">{{ data.title }}</p>
        <ul class="flex gap-4 time-list">
      ...
      <div class="product mb-4 w-full max-h-[480px] relative group overflow-hidden">
        ...
        <div class="cursor-pointer">
          <img
            :src="data.image"
            alt=""
            class="transition-transform duration-500 block w-full h-full group-hover:scale-110"
          />
        </div>
        <!-- product icon -->
        ...
        <!-- product discount -->
        <div
          class="flex justify-center items-center w-7 h-7 absolute bottom-2 right-1 bg-secondary-dark/60 rounded-full"
        >
      ...
      <div class="flex flex-col gap-4">
        ...
        <div class="flex justify-between text-xs text-light-text">
          <div>
            <span class="mr-1">Stock:</span>
            <span class="font-bold">{{ data.stock }}</span>
          </div>
          <div>
            <span class="mr-1">Sold:</span>
            <span class="font-bold">{{ data.sold }}</span>
          </div>
        </div>
        <div class="h-1 bg-border overflow-hidden rounded">
          <div class="h-1 bg-secondary" :style="`width: ${progress}%;`"></div>
        </div>
      </div>
    </div>
  </div>
